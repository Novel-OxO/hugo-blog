...
이러한 비효율을 개선하기 위해 등장한 개념이 바로 **버퍼(buffer)**입니다. 매번 곱바로 하위로 내려보내는 대신, 데이터를 일단 메모리의 임시 저장 공간(버퍼)에 모아두었다가, 충분히 쌓였을 때 한 번에 내려보내자는 발상입니다. 작은 쓰기 여러 번을 큰 쓰기 한 번으로 합치면, 옮기는 데이터량은 그대로이면서 비싼 전환 횟수만 크게 줄일 수 있습니다.
